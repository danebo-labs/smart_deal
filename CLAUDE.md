...
- **`RagRetrievalProfile`** (app/services/rag_retrieval_profile.rb): adaptive `number_of_results` from pin signal and intent. Photos-only→10, docs/mixed focused→3, stop/failure/repair→5, no-pin→8, no-pin schematic block/connector query→20 (`MAX_RESULTS`, surfaces the block→connector overview chunk; == `ContractualLimits::QUERY[:max_top_k]`), exhaustive checklist→15 candidates (optionally reranked to 12). Derived from the URI-aligned pinned entity subset in `QueryOrchestratorService` with zero extra DB queries.
