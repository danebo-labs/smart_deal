...
- **Document processing**
- **Bulk ZIP ingestion (signed-in web)** — **`/bulk_uploads`**: fast controller ACK → Solid Queue **`bulk_ingestion`** lane extracts ZIP, uploads to S3, submits **Anthropic Message Batches** (**`claude-opus-4-7`**, **`BatchChunkingPrompt`**) for **server-side queued** processing, then reads **streamed JSONL** batch results (not live chat token streaming), parses into chunk `.txt` files with pipeline-injected identity headers, syncs via optional **`BEDROCK_BULK_DATA_SOURCE_ID`**, polls Bedrock ingestion, links **`KbDocument`**, and **Turbo Streams** asset rows on the show page. See [Bulk ZIP ingestion (web)](#bulk-zip-ingestion-web).
- **Web upload custom chunking** (`CUSTOM_CHUNKING_WEB_ENABLED=true`) — feature-flagged optimized path for chat file attachments. Bypasses `OWRPGSX6XK` (FM-parsing + Lambda). Rails calls Anthropic Messages API directly with model routing (`claude-sonnet-4-6` for text, `claude-opus-4-7` for images/multimodal PDFs), writes pre-processed chunks to the bulk data source (chunking=NONE), and falls back to legacy OWRPGSX6XK on any error. Includes `PageRelevanceFilter` (heuristics + Haiku 4.5 gating) and image-density-based conservative downgrade for PDFs. Deploy with flag OFF → smoke-test staging → enable in prod via `CUSTOM_CHUNKING_WEB_ENABLED=true` (Kamal env push).
