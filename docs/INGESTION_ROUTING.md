...
Per-page PDF parses use `BatchChunkingPrompt::INGESTION_CONTRACT_VERSION` (`field_records_v7`) with O4a page roles. The first kept page is tagged `ANCHOR_PAGE` and emits S0, `summary`, and `companion_offer`; remaining kept pages are tagged `CONTENT_PAGE`, omit that repeated metadata, and keep `document_name` / `aliases` for Rails merge and fallback logic. Minimal validation passed on 2026-06-16 with batch `msgbatch_01BpMauhuRC7GDePzWQMU27f` (2 pages, USD 0.04736). O4b offline review did not justify a prompt/contract v5 or paid shadow: `tmp/o4a_min_batch_shadow.json` showed the CONTENT_PAGE still had higher output tokens than the anchor, so remaining safe metadata savings are below the original target. The only O4b action kept is deterministic Rails cleanup: sync page prompts now use the kept-page count after filtering, and `ChunkMergerService` drops accidental `S0 — DOCUMENT IDENTIFICATION` chunks from non-anchor pages before indexing — but only when the page has other chunks, so a brand/section divider page whose single chunk is identification-shaped is never deleted.

### Retrieval identity rules (2026-07-25)

Two deterministic rules protect brand-level retrieval on multi-manufacturer compendiums:

- `PageRelevanceFilter.section_identity_guard?` overrides a Haiku drop whose reason names a divider or index unless the page's readable text layer confirms a page-number table of contents. On a scanned compendium the divider page is the only place a manufacturer or controller family is written, so dropping it makes every page of that section unreachable by brand. The batch classifier prompt also keeps any page naming a manufacturer, controller family, or model codes.
- `ChunkMergerService` reports `document_name_consensus`. When no page name holds a majority — a compendium where each section names its own board — `BatchResultsParserService` takes the document identity from the uploaded filename instead of labelling every chunk with the anchor page's board. `canonical_name` in the sidecar, the asset, and every `doc_ref` stay consistent.

Contract `field_records_v5` additionally requires a component and its supply/voltage label on the same line, so a diagram value can never be read as belonging to a neighbouring component. `field_records_v6` forbids a 1:1 component assignment on a numbered terminal block (series chains and 2-pole devices are emitted as a terminal range with `REQUIRES_FIELD_VERIFICATION`).

`field_records_v7` closes the third leg of the compendium problem: keeping the divider page in the index is not enough, because the brand is printed only there. A page that visibly opens a brand/controller-family section emits top-level `section_identity` (verbatim visible text, never inferred), `ChunkMergerService` carries the last declared value forward in page order into every following page's chunk aliases until another page declares a new one, and `BatchResultsParserService` writes it into the `[SEARCH_ALIASES: …]` line plus a `section_identity` sidecar attribute. Bumping the contract does not re-parse already-indexed documents.
