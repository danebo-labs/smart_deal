...
| **`.kamal/secrets`** | Dotenv-style file at the **project root** (same level as `Gemfile`). Kamal reads it when you deploy; it must define every name listed under `secret:` and `registry.password` in `deploy.yml`, for example: `RAILS_MASTER_KEY=...`, `DB_PASSWORD=...`, `ANTHROPIC_API_KEY=...`, `GROQ_API_KEY=...`, `OPENAI_API_KEY=...`, `APPSIGNAL_PUSH_API_KEY=...`, `KAMAL_REGISTRY_PASSWORD=...` (use a **Docker Hub access token**, not your main password, if possible). **`GROQ_API_KEY` is not optional** while `STT_PROVIDER=groq`: the transcription layer has no automatic failover, so without it every voice dictation lands in `failed`. |
...
|------|----------------------------------|
| [`config/deploy.yml.example`](../config/deploy.yml.example) | **Kamal:** `service: smart-deal`, **one host** runs both **`web`** (Puma, `memory`/`cpus` limits) and **`worker`** (`bundle exec rake solid_queue:start`). **`proxy`:** TLS, public host, `app_port: 80` to Puma. **Registry:** Docker Hub (`KAMAL_REGISTRY_PASSWORD` in `.kamal/secrets`). **`env.clear`:** `RAILS_ENV`, logging/static flags, `RAILS_MAX_THREADS`, `AWS_REGION`, `DB_HOST` / `DB_USERNAME` / `DB_NAME` / `DB_{CACHE,QUEUE,CABLE}_NAME`, Bedrock KB + bulk data source + model IDs, `KNOWLEDGE_BASE_S3_BUCKET`, `INGESTION_REENQUEUE`, `AWS_HTTP_READ_TIMEOUT`, `QUERY_ROUTING_ENABLED`, `SHARED_SESSION_ENABLED`, `SHOW_RAG_SOURCES`, the three `RAG_EVIDENCE_*` selector flags, `RAG_STRUCTURED_EVIDENCE_ROUTE_ENABLED`, and the two contract flags `RAG_CITATION_ATTRIBUTION_CONTRACT_ENABLED` / `RAG_PARTIAL_ABSTENTION_CONTRACT_ENABLED` (only the exact string `"true"` enables them; both default to `"false"`, which is the pre-contract behavior — see [RAG_CITATION_ATTRIBUTION_CONTRACT_2026-07-30.md](RAG_CITATION_ATTRIBUTION_CONTRACT_2026-07-30.md)), and `PHOTO_QUESTION_RAG_ENABLED` (default `"false"`; when `"true"`, `FieldPhotoAnalysisJob` runs a text-RAG turn anchored to the just-analyzed photo whenever the technician's upload also carried a question — `Rag::PhotoQuestionAnswerService`). **`secret`:** `RAILS_MASTER_KEY`, `DB_PASSWORD`, `ANTHROPIC_API_KEY`, `GROQ_API_KEY`, `OPENAI_API_KEY`, `APPSIGNAL_PUSH_API_KEY`. Also `STT_PROVIDER` in `env.clear` — the one line that moves transcription provider without shipping code (`groq` default; see [PLAN_IMPLEMENTACION_CERTIFICADOR_2026-08-09.md](PLAN_IMPLEMENTACION_CERTIFICADOR_2026-08-09.md) section 4). **SSH** user + key path. |
