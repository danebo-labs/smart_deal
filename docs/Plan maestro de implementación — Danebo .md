...
| FC-D11 | Antes de generar, si el episodio conoce la marca, un chunk de otra marca entra solo con manual, página y título de sección, sin su texto. Si la marca del trabajo no se conoce, no se filtra y se pregunta la marca. La marca del chunk no sale de `[SEARCH_ALIASES:]`. | Dueño, 23-sep-2026 | **Implementada detrás de flag apagada.** No se puede activar mientras una entrada del corpus general no tenga `confirmed: true`. |
| FC-D12 | Identidad en `config/document_identities.yml`. Clave `account_id` + `document_id` (el sidecar guarda `original_source_uri` = `s3://bucket/` + `s3_key`, así que la clave no es `s3_key`). Ids de índice: 1 danebo-legacy, 3 danebo-pilot-elevator. Match exacto con `FollowupQueryRewriter.normalize_label` entre brands ∪ designators y manufacturer ∪ model ∪ identifiers. Solo si `facts.manufacturer` está `known`. Declarado y coincide, o genérico: cuerpo completo. Declarado y no coincide: «Reference only, other equipment» con manual, página y título (el título se omite si tiene dígito o barra). Manual de otra cuenta: cuerpo completo y `undeclared_private`. Enmienda 23-sep: `confirmed: true` solo si el documento imprime la marca (portada, primera página, rótulo del plano o encabezado), con `evidence_page` y `evidence_text` tal como está impresa. No cuentan la memoria del modelo, los alias ni `[SEARCH_ALIASES:]`. Sin marca impresa queda `confirmed: false`. Una entrada sin confirmar no bloquea el turno: el chunk entra con su cuerpo y telemetría `unconfirmed_general`. Solo se recorta un documento confirmado que no coincide. Un manual de la cuenta que no está en el YAML entra completo con `undeclared_private`. `role` `equipment` se recorta si la marca no coincide; `component` solo si hay identificador y ninguno coincide; sin `role` no se recorta. Match exacto normalizado, nunca substring. Activable cuando el YAML carga y hay al menos una entrada confirmada con marca impresa. `DOCUMENT_IDENTITY_SCOPE_ENABLED` apagada. Con la flag efectiva, un Retrieve con el mismo top_k y rerank que `retrieve_and_generate`. Si nada se recorta, el turno sigue ahí. Si algo se recorta, la generación conserva la cantidad y el orden y solo reemplaza ese cuerpo. Excepción o timeout: vuelve el camino de hoy. No usa `select_generation_chunks` ni `MAX_CONTEXTS`. Mismo modelo Haiku 4.5. `FIELD_COMPANION_TURN_ENABLED` no se enciende. | Dueño, 23-sep-2026 | **Enmendada 23-sep. Flag apagada.** Medición pendiente de la marca impresa. |
