...
3. **Locale-aware summaries** — whole-file/image parses emit `summary` and `companion_offer` normally. Per-page PDF parses use O4a page roles: the first kept page is `ANCHOR_PAGE` and emits S0, `summary`, and `companion_offer`; later `CONTENT_PAGE` calls omit those fields and still emit top-level `document_name` / `aliases` for Rails identity fallback. `ChunkMergerService` extracts the anchor page's metadata and uses deterministic Rails fallbacks if the anchor is degraded.
4. **Compact manual evidence** — Sonnet/Opus emit short-key `field_records` only for qualifying evidence and omit absent optional fields. Rails validates, assigns deterministic IDs, and expands canonical labels before S3 upload; no additional LLM call is introduced. Manual chunks produced by `BatchChunkingPrompt` use the current `field_records_v7` contract.
