...
modelo que escribe el código, que consume mucho más que las mediciones. Ver el
[Apéndice H — presupuesto y saldo](#apéndice-h--presupuesto-saldo-y-cómo-no-quedarse-a-medias).

**Los prompts listos para copiar y pegar de estos tres pasos están en el Apéndice G**, al final,
bajo `Fase 5b`, `Medición de v3` y `Fase 7`.
...
| Imagen densa sin vectores / sin capa de texto | **T2 visión** | Opus 4.8 + prompt de relaciones sobre ráster + crops | **$0,0675-0,0796/página, medido** ² | `TOPOLOGY_EDGE` `method: vision` |
...
² ⚠️ **medido en I-34.** Ya no es una estimación: 3 páginas corridas en vivo con Opus 4.8 por API
directa dan $0,0675-0,0796 por página (≈5 400 tokens de entrada — página a 150 DPI más 11-15 crops a
200 DPI — y ≈1 700 de salida), es decir ~**$5,40** por las 80 páginas T2 de este documento. ⚠️
**precisado en I-38:** ése es el precio **siempre**. La visión sale por la Messages API directa
incluso cuando la ingestión que la rodea usa el Batch API, porque tiene que resolverse antes de
enviar el batch, mientras el binario de la página existe. El 50 % de descuento del Batch se lo
llevan las llamadas de chunking. Tener el descuento en visión exigiría un segundo batch propio para
T2 (~$2,70), que nadie ha construido.
...
| **5b-run** | Lanzar la corrida con teselas | **ninguno — hazlo tú en la terminal** | Es un comando. Pasarlo por un agente cuesta más en tokens del agente que la propia medición |
| **5b-juicio** | Juzgar las relaciones contra la lámina | **Opus** | Visión + criterio; es el trabajo caro y el que decide el veredicto. Sonnet aquí ahorra céntimos y arriesga el gate |
| **v3-medición** | Puntuar el prompt v3 | **Opus** | Mismo juicio visual; se puede encadenar con 5b-juicio en la misma sesión y compartir la lectura de láminas |
...
2. Si un hallazgo invalida una premisa de una fase posterior, **edita esa fase en el sitio** —
   tanto su sección bajo "Plan por fases" como, si existe, su prompt en el Apéndice G — y marca el
   cambio con `⚠️ revisado en I-NN`. **La columna "Impacto en fases posteriores" del Registro no
   es el traspaso, es sólo el índice de que hace falta uno.** Escribir ahí "Fase X: hacer Y" y no
   ir a editar la sección de la Fase X dice que hiciste el trabajo sin haberlo hecho — es el mismo
   fallo que I-23 registró para los commits, aplicado a la prosa en vez de al git log.
...
5. **Antes de entregar, releé tu propia entrada del paso 1.** Por cada fase que nombra en su
   columna de impacto, abre esa sección (y su prompt si tiene uno) y confirma que el marcador
   `⚠️ revisado en I-NN` ya está escrito ahí — no en la memoria de que "ya lo anoté en algún
   lado". Si falta, es tu entrega la que está incompleta, no la de quien ejecute esa fase después.
...
GATE A (fallido) ──▶ 2b ✅ ──▶ 3b ✅ ──▶ GATE A-bis ✅ ──▶ [decisión #4] ✅ ──▶ 4 ✅ ──▶ 5 ✅ ──▶ GATE B ──▶ 7
                     f4ab397     1cb789b      100 %, 0 falsas   opción B         9f9d611  2b3ff19     ↑
                                                                                                    AQUÍ
```
...
### `VisionTopologyExtractor.derive(page_binary, layout:, …)` → Fase 5 produce, Gate B y Fase 7 consumen

```ruby
#<struct Result
  edges: [
    { from: "32", to: "CERROJOS CABINA", method: :vision,
      evidence: "conductor naranja que sale de la borna 32 y llega al gráfico rotulado CERROJOS CABINA" }
  ],
  components: [
    { label: "SOBRECARGA", canonical_component: "unidad de control de carga",
      evidence: "rótulo MICELECT MWR-4 visible en la foto" }
  ],
  crop_count: 14, input_tokens: 5570, output_tokens: 2069>
```

Reglas: `edges` tiene **exactamente** la forma que consume la Fase 4 (`from`/`to`/`method`/
`evidence`, sin `chain`) y se concatena al array de T1 en el mismo `page_result[:topology_edges]`;
`method` es siempre `:vision`. Igual que en T1, **`from`/`to` no son una dirección** (I-11) **ni son
exhaustivos** (I-29), y una etiqueta que no resuelve simplemente no aparece: `[]` es salida válida.
A diferencia de T1, un extremo **puede no tener ninguna letra** — los números de borne impresos
dentro del ráster son justo lo que este tier existe para leer (I-15). `components` **no** se
renderiza en ningún chunk en esta fase: existe para que el Gate B pueda medir la capacidad
...
### Fase 5 — Motor T2: visión sobre páginas triadas · Opus — ✅ **CERRADA**
...
**$0,0675-0,0796/página**, ~**$5,40** por las 80 páginas T2 del documento. **Ese precio es de API
directa y no baja**: la llamada de visión sale por `ClaudeChunkingClient` (Messages API en streaming)
porque tiene que resolverse **antes** de enviar el batch, mientras el binario de la página existe. Por
la ruta asíncrona el 50 % de descuento del Batch API se lo llevan las llamadas de *chunking*, no las
de visión. Correr T2 por su propio batch costaría ~$2,70 y es trabajo nuevo que esta fase **no** hizo
(ver I-38).
...
- [x] Rasterizador con test sobre el fixture; DPI justificado, no mágico — `PdfPageRasterizer`,
      `PAGE_DPI = 150` (la resolución a la que se leyó la verdad-terreno humana del Gate A/A-bis)
      y `CROP_DPI = 200` (p50 198,7 / p90 202,6 de densidad nativa de colocación de las 1 646
      imágenes pequeñas del documento), con test de la conversión de coordenadas y del degradado
      de DPI
- [x] Crops anclados a su etiqueta adyacente, con test — `TopologyEdgeDeriver.label_for_image`,
      la regla anti-ráster de la Fase 3b leída **hacia adelante** en vez de reimplementada al lado
      (I-20). Medido: 1 345 de 1 646 imágenes pequeñas resuelven etiqueta (81,7 %), p50 15
      crops/página
- [x] Prompt de relaciones emitiendo v8 `method: vision`, con test de parseo — `from`/`to`/
      `evidence` reusados de `FieldPhotoPrompt`, sin schema nuevo
...
⚠️ **añadido en I-46 — el dueño preguntó de dónde salía el 85 % y la respuesta honesta es que de
ninguna derivación.** Fue un número razonable puesto a ojo. Medido el gate, esto es lo que se puede
decir con datos, y **la conclusión no es bajarlo**:

- **85 % es "1 de cada 7 relaciones mal".** Para un dato que el sistema **afirma** a un técnico, eso
  ya es alto, no bajo. Bajarlo a 80 % dejaría pasar v1 (LI 81,6 %) — pero el modo de fallo medido es
  el borne vecino, o sea el error que el técnico cablea. No es un fallo que se note; es un fallo que
  se ejecuta.
- **El número correcto no depende de la estadística sino de qué hace el sistema con la relación.**
  Tres regímenes distintos, y el umbral debería ser distinto en cada uno:
  1. **La relación se cita como instrucción** ("el borne 71 va a CERROJOS EXTERIORES") → hace falta
     más del 95 %. 85 % ya era generoso.
  2. **La relación se cita con calificador obligatorio** ("lectura de visión, verifíquela en la
     lámina") → 85 % es razonable, porque el coste del error deja de ser cablear mal y pasa a ser
     mirar la lámina. **Esto no existe hoy**: habría que construirlo en la Fase 6.
  3. **La relación no se cita nunca y sólo sirve para recuperar el chunk correcto** → la precisión
     casi no importa; incluso al 70 % ayuda al recall y no puede inducir a error.
- **Y hay una palanca mejor que mover el número: filtrar por tipo de lámina.** Los tipos A y B juntos
  dan **37 de 37, LI 92,2 %** — superan cualquier umbral razonable. El problema está localizado en el
...
del régimen 3. Cambiar el número sin cambiar el uso es cambiar la vara para que el resultado pase.
...
⚠️ **añadido en I-37, corregido en I-38.** Y una quinta: T2 corre **por la ruta asíncrona**
(`ManualBatchIngestionService`), que es la única que producción usa — pero **la llamada de visión en sí
NO va por el Batch API**. Sale por `ClaudeChunkingClient` (Messages API en streaming) desde
`SubmitManualBatchJob`, porque tiene que resolverse antes del envío del batch, mientras el binario de la
página existe. Así que **T2 cuesta API directa siempre, ~$0,0796/página, con ruta asíncrona o sin ella**;
el 50 % de descuento del Batch se lo llevan las llamadas de chunking. Y la corrida tarda del orden de
40-50 minutos en enviarse porque esas llamadas van en serie. Medir llamando a
`VisionTopologyExtractor.derive` desde tu propio script no cambia el precio unitario — sólo te deja
...
cualquier orden y sin distinguir mayúsculas. Este gate la **confirma o la cambia**, no la escribe
desde cero. (b) El **coste por página está medido**: $0,0675-0,0796 en API directa (~5 400 tokens
de entrada, ~1 700 de salida por página), ~$5,40 por las 80 páginas T2 del documento. **T2 siempre
paga API directa**, tenga la ingestión detrás la ruta de Batch o no (I-38): el descuento del Batch API
cubre el chunking, no la visión. Lo que falta es el coste **total** de la corrida que hagas aquí. (c) `VisionTopologyExtractor`
loguea una línea `vision_topology_page` por página con tokens, DPI, bytes, número de crops, embudo
de rechazos y **huella del prompt** — el embudo del Gate A, para T2. (d) **T2 no ve las aristas de
T1**, por diseño y con test: `traced_edges` sólo sirve para deduplicar. Si algún día se le pasan al
prompt, esta medición deja de medir a T2 y empieza a medir a un copista.

**Trabajo nuevo que este gate hereda.** (e) T2 compone extremos que **no están impresos
literalmente**: en la página 63 emitió `J10-3`, `J12-7`, `J23-1` — el conector y el número de borne
existen impresos por separado, la cadena unida no. Es más útil para un técnico y a la vez es un
token que no existe en el documento, así que decide **explícitamente** si se normaliza (extremo =
número de borne, conector en `evidence`), se acepta como convención documentada, o se prohíbe en el
prompt. Ver I-36. (f) La misma página 63 emitió `J10-3` conectado a **dos** dispositivos distintos
con colores de cable distintos en la evidencia (amarillo y rojo): al menos uno es falso, y es el
primer indicio medido de que la precisión de T2 **no es uniforme** — la página 17 (regleta frontal,
cables cortos) se lee bien; la 63 (dos conectores, cables largos que cruzan la lámina) no. Mide por
tipo de lámina, no en agregado.
...
**La hipótesis, en una línea.** Los 9 errores de "celda vecina" que hundieron el gate ocurren donde
la fila de bornes ocupa 40-60 px de la página renderizada a 150 dpi, y **todos se resolvieron sin
ambigüedad al ampliar a 300 dpi durante el juicio humano**. La ampliación que el juez necesitó es la
que el modelo nunca tuvo.

**Por qué no hay detector, que es un hallazgo en sí (I-44).** Lo primero que se intentó fue localizar
la regleta por geometría y recortarla. No se puede: los nombres de las celdas (`SE5 SE6 SE7`,
`109 111 112`, `B1…B6`) **no están en la capa de texto ni en los `rects`** — son píxeles dentro de la
foto de la regleta. Medido: la página 78 tiene 35 palabras y **ninguna** es un nombre de celda; sus
`rects` son marcos de recorte de página, no celdas. Es la misma causa que I-15 le encontró a T1,
vista desde el otro lado. Así que no se detecta nada: **se parte la página en teselas y se mandan
todas**, y el que localiza es el modelo.

**Qué se implementó** (commit al pie de la tabla de fases):

- `PdfPageRasterizer#tiles` — 3×2 teselas con 10 % de solape a `ZOOM_DPI = 300`. En este documento
  (960×540 pt) son 6 imágenes de ~1500×1271 px. El solape existe para que una regleta que cae en un
  corte siga entera en alguna tesela.
- `VisionTopologyPrompt` las envía tras la página completa, etiquetadas `ZOOM n of 6`, diciendo
  explícitamente que **no añaden contenido**, sólo resolución para distinguir una celda de su vecina.
- Flag propio `INGESTION_VISION_TIER_ZOOM_TILES`, **apagado**. No cambia nada hasta que alguien lo
  encienda.
- **Corregido de paso, I-44:** el tope `MAX_LONG_EDGE_PX` se aplicaba al render de página completa
  también cuando lo que se emitía era un recorte, así que en este documento **todos los recortes de
  componente se venían rasterizando a 150 dpi y `CROP_DPI = 200` no significaba nada** desde la
  Fase 5. Ahora cada recorte tiene su propio techo. Sube la resolución de los recortes de componente
  sin que nadie lo pidiera: es parte de lo que hay que volver a medir.

**Verificación hecha sin gastar API:** se volcó a disco la tesela 4 de la página 78 y se leyó con
visión. `SFH SNH SE5 SE6 SE7 SE8 SE9` salen perfectamente legibles y se ve que el verde sale de
`SE5`, el amarillo de `SE6` y el rojo de `SE7` — exactamente los dos datos que T2 falló dos veces en
esa página. El experimento está bien planteado; lo que falta es puntuarlo.

**Predicción falsable, escrita antes de medir.** Si la hipótesis es cierta:

- el tipo C (regleta densa) sube de **81,5 %** hacia el 95 %;
- el tipo A y el tipo B **no se mueven** (ya están al 100 %);
- la precisión agregada supera el 85 % con el límite inferior por encima del umbral;
- el coste por página sube de $0,065 a ~**$0,14**.

Si el tipo C **no** se mueve, la hipótesis está refutada y el veredicto del Gate B es definitivo: el
límite no es de resolución sino de la capacidad del modelo para seguir un conductor, y la degradación
a campos no-relacionales pasa de ser provisional a ser permanente. **Ese resultado negativo también
...
**Cuidado con el precio: la visión NO se abarata por ir dentro de la ruta de Batch (I-38).** La
llamada de T2 sale por `ClaudeChunkingClient` (Messages API) desde `SubmitManualBatchJob`, antes del
envío del batch, así que paga **API directa**: ~**$5,40** por las 80 páginas T2 del documento, no
~$2,70. El 50 % de descuento del Batch cubre el chunking. Presupuéstalo y déjalo en el informe; no es un
coste que la Fase 4 tuviera. Cuenta también el tiempo: esas llamadas van **en serie**, del orden de
40-50 minutos para 98 páginas (I-37 (e)); si molesta, la mitigación ya escrita es el patrón
`chunk_pages_parallel` que existe en la ruta síncrona. Y si quieres el precio de Batch para la visión,
eso es un batch propio para T2 — trabajo nuevo, ver I-38.
...
para T1 porque su `evidence` es una cadena determinista. La `evidence` de T2 es prosa de un modelo:
la misma página re-ingestada produce otra redacción y por tanto **otro `RECORD_ID`**. Consecuencias
concretas para esta fase: la corrida de 6 páginas y la de 98 **no** comparten IDs de registro de
topología para las aristas de visión; un re-run del shadow ingest no es un no-op; y el "delta de
...
⚠️ **revisado en I-35 e I-36.** El bullet de arriba —"los `required` son cadenas de etiqueta
verbatim emitidas por el digest"— vale para T1 y **no** para T2. Tres cosas que no se pueden
convertir en `required` sin más: (a) la `evidence` de una arista `vision` es **prosa de un modelo, no
determinista**, así que un `required` que la cite literalmente falla en la siguiente corrida (I-35);
usa los **extremos**, que sí son transcripción; (b) desde I-34 esa prosa se pide en el idioma de la
página (`locale:`), así que tampoco es estable frente a un cambio de locale; (c) los extremos de T2
**no siempre están impresos verbatim**: puede emitir `J10-3` donde la página imprime `J10` y `3` por
...
> que tu hallazgo invalide, en su sección **y** en su prompt de este Apéndice si tiene uno; tu fila
> en la Tabla de estado de fases con el hash real; y, **antes de entregar**, releé tu propia
> columna "Impacto en fases posteriores" y confirma que cada fase que nombraste ya tiene el
> marcador `⚠️ revisado en I-NN` escrito donde corresponde. Escribir el impacto en esa columna sin
> ir a editar la fase referida dice que hiciste el traspaso sin haberlo hecho.
...
> implementada** (`VisionTopologyExtractor#drop_traced`): la confirmas o la cambias, no la escribes.
> **(2)** El coste por página **ya está medido** en vivo: $0,0675-0,0796 (~5 400 tokens de entrada,
> ~1 700 de salida), ~$5,40 por las 80 páginas T2 en API directa; lo que falta es el coste de tu
> propia corrida. **(3)** Hay una línea de log `vision_topology_page` por página con tokens, DPI,
> crops y **embudo de rechazos** — el embudo del Gate A, para T2 — y la huella del prompt, que es lo
> que hace atribuible cada medición a la versión exacta del texto que la produjo. **(4)** T2 **no ve**
> las aristas de T1 y no debe verlas: es lo que hace que esta medición mida a un lector y no a un
> copista. Además hereda dos problemas medidos que sí son tuyos: extremos **compuestos** que no están
> impresos (`J10-3`, I-36) y **precisión desigual por tipo de lámina** (la página 17 se lee casi
...
> real, la llamada de visión **igual paga API directa** (I-38): sale por ClaudeChunkingClient antes del
> envío del batch, así que el 50 % de descuento del Batch cubre el chunking y no la visión — ~$0,0796 por
> página en cualquier caso, ~$5,40 por las 80. Y las llamadas van en serie antes del envío (~40-50 min
> para 98 páginas). Medir llamando a `VisionTopologyExtractor.derive` desde tu propio script no cambia el
> precio unitario; sólo te deja controlar el paralelismo.
>
> 💰 **Presupuesto fijado por el dueño del producto: $15 en total, y no lo excedas sin preguntar.**
> Con el coste medido ($0,0796/página, API directa) eso son ~188 corridas de página. El conjunto de
> medición son **~25 páginas, no 80**: las **11** con verdad-terreno humana (153 relaciones, Gate A
> §5) más las **18** donde T1 emite algo, con solape. Una pasada sobre esa unión cuesta ~**$2**, así
> que el presupuesto da unas **7 pasadas**: reparte entre iteración del prompt, la corrida final
> puntuada y 2 repeticiones para estimar la varianza de I-35. **Itera en API directa** (segundos por
> página). Ojo: **no hay versión barata de la visión** — no existe hoy una ruta de Batch para T2
> (I-38), así que una pasada sobre las 80 páginas cuesta ~$6,40 y se comería el 43 % del presupuesto.
> Piénsalo dos veces antes de hacerla; el conjunto de ~25 páginas es donde está la señal. Registra el
> gasto real en el informe.
>
> ⚠️ **El límite de este gate no es el dinero, es el criterio humano.** En las ~69 páginas sin
> verdad-terreno, T2 va a emitir relaciones que nadie puede juzgar sin mirar la lámina con visión. Los
...
> entera, el **§10 y el §2 del informe `docs/rag/gate_b_calibracion_vision.md`**, y las entradas
> **I-39, I-40 e I-44** del Registro. El código **ya está escrito y testeado**: `PdfPageRasterizer#tiles`,
> el flag `INGESTION_VISION_TIER_ZOOM_TILES` y el bloque `ZOOM n of 6` del prompt. **No lo reescribas.**
> Tu trabajo es **medir y juzgar**, que es lo único que falta.
>
> Corre exactamente esto:
> ```bash
> GATE_B_ZOOM_TILES=true \
> GATE_B_PAGES=3,11,12,14,17,22,25,39,44,52,56,61,63,64,67,76,77,78,91,93,94,95,97 \
> GATE_B_OUT=tmp/gate_b_5b.json GATE_B_LABEL=fase5b GATE_B_THREADS=4 \
>   bin/rails runner script/gate_b/run.rb
> ```
> Después **juzga a mano**, con visión, contra la lámina renderizada a 150 dpi
> (`pdftoppm -f N -l N -r 150 -png "<PDF>" tmp/gate_b_png/pN`), ampliando la fila de bornes a 300 dpi
> cuando haga falta —`pdftoppm -r 300 -png -x X -y Y -W W -H H`—, con **la regla de juicio del §1.2
> del informe, sin cambiarla**: correcta = hay conductor entre los dos extremos, directo o por
> intermedios **que la propia `evidence` nombra**. Nunca juzgues contra la salida anterior del modelo.
>
> Publica el desglose **por tipo de lámina** (A: regleta frontal tipo pág. 17 · B: conectores
> pequeños tipo pág. 97 · C: regleta densa tipo págs. 63/78/93) al lado de los números del §2 del
> informe, con el límite inferior de Clopper-Pearson al 95 % de cada uno. La predicción escrita antes
> de medir es que **sólo el tipo C se mueve**; si no se mueve, la hipótesis está refutada y **eso
> también hay que escribirlo**, con esas palabras.
>
> Enciende `INGESTION_VISION_TIER_ZOOM_TILES` **sólo** si el límite inferior agregado supera el 85 %.
...
> `vision_topology_v3` (huella `36f8c3bb3a8d…`) está en el repositorio **como texto activo y sin una
> sola llamada medida**: se escribió cuando la cuenta se quedó sin saldo. Mídelo antes de que nadie
> se fíe de él.
>
> ```bash
> GATE_B_PAGES=3,11,12,14,17,22,25,39,44,52,56,61,63,64,67,76,77,78,91,93,94,95,97 \
> GATE_B_OUT=tmp/gate_b_v3.json GATE_B_LABEL=v3 GATE_B_THREADS=4 \
>   bin/rails runner script/gate_b/run.rb
> ```
> Misma regla de juicio del §1.2, mismo desglose por tipo. Compara contra v1 (§2) y contra v2 (§8.2).
> **Si v3 no mejora a v1, vuelve el prompt a v1**: una versión sin medir en producción es deuda, y v1
> es la única con 102 relaciones juzgadas detrás. Sea cual sea el resultado, quita la advertencia de
> "SIN MEDIR" del comentario de `CONTRACT_VERSION` y sustitúyela por el número.
>
...
> puede emitir `J10-3` donde la página imprime `J10` y `3` por separado (I-36). Para una arista
> `vision`, usa los **extremos** y sólo después de que el Gate B decida si se normalizan. Y de I-30:
> si un caso espera una respuesta que cite una arista `vision`, la respuesta esperada necesita el
> calificador de imagen o el guardia la degrada antes de la rúbrica.
>
...
## Apéndice H — Presupuesto, saldo y cómo no quedarse a medias

Escrito porque el Gate B se cortó por esto y no por su presupuesto (I-42): **$2,2216 gastados de los
$15 autorizados, y aun así la API devolvió `credit balance is too low`.**

### La causa: una sola clave para dos consumos muy distintos

| Consumo | Qué es | Orden de magnitud |
|---|---|---|
| **Medición** | las llamadas de visión de T2 que este plan puntúa | **$1,50-3,30 por pasada** de 23 páginas |
| **El modelo que escribe el código** | la sesión de ingeniería que lee ficheros, edita, corre tests y juzga láminas con visión | **decenas de dólares** por sesión larga |

Los dos salen de la misma `ANTHROPIC_API_KEY` si no se separan, y el segundo se come al primero sin
avisar. Recargar de $10 en $10 garantiza que la próxima medición se quede a mitad.

### Qué hacer, en orden de preferencia

1. **Separar las claves.** Una clave (o un workspace aparte en la consola de Anthropic) **sólo** para
   la aplicación y las mediciones, otra para el modelo que escribe el código. Es un cambio de
   configuración, cuesta $0, y hace imposible que una sesión de ingeniería agote el presupuesto de
   una medición. **Es la recomendación.**
2. **Sacar el asistente de código de la API.** Si se le puede pagar por suscripción en vez de por
   consumo de API, deja de competir por este saldo y la clave de la aplicación se vuelve predecible.
3. **Si tiene que ser una sola clave:** cargar antes de empezar **el coste de la sesión de ingeniería
   más el de la medición**, no sólo el de la medición.

### Coste medido de cada paso pendiente
...
```bash
GATE_B_PAGES=17 GATE_B_OUT=tmp/saldo.json GATE_B_LABEL=saldo bin/rails runner script/gate_b/run.rb
```

Si sale `T2=0 comp=0 crops=0 $0.0` en dos segundos, **no es un bug: es el saldo**.
`VisionTopologyExtractor#derive` rescata la excepción a propósito —un tier que no puede leer una
página no debe tumbar la página—, así que una corrida sin saldo **termina bien, con cero aristas y
coste cero**, y sólo se distingue leyendo la línea `VisionTopologyExtractor: pN failed` del log.
...
| I-33 | 4 | Sonnet 5 | **El Protocolo de traspaso se cumplió a medias en la primera pasada de esta fase, y el propio preámbulo común de los prompts no bastaba para evitarlo.** Cerré I-31 (el hueco de la ruta asíncrona) escribiendo su "Impacto en fases posteriores" con instrucciones concretas para la Fase 5 y la Fase 7 — y di la fase por entregada sin ir a editar esas dos secciones. El dueño del producto lo notó y preguntó explícitamente si el traspaso se había ejecutado; sólo entonces se hizo la edición en el sitio (Fase 5, Fase 7 y su prompt en el Apéndice G, más el prompt histórico de la propia Fase 4). El "Preámbulo común" del Apéndice G ya decía "edición en el sitio de las fases que invalides", pero una frase de una línea, sin un paso de auto-verificación, es fácil de dar por satisfecha con sólo la columna de impacto del Registro — que documenta la intención, no la ejecuta. Se reforzaron dos sitios: el paso 2 del Protocolo de traspaso (§"Modelo recomendado por fase") ahora dice explícitamente que la columna de impacto **no es** el traspaso, y se añadió un paso 5 de auto-verificación obligatoria antes de entregar; el preámbulo común del Apéndice G se reescribió con la misma exigencia y se extendió a cubrir también los prompts del propio Apéndice G, no sólo las secciones de "Plan por fases" (el prompt de Fase 4 citaba una `section_path` de 2 niveles que ya no era cierta, y nadie lo había tocado). | **Toda fase futura:** el paso 5 nuevo del Protocolo de traspaso es una lista de verificación literal — antes de entregar, releer la propia columna de impacto que acabas de escribir y confirmar con un grep (`revisado en I-NN`) que cada fase nombrada ya quedó editada, no darlo por hecho porque "ya lo anoté". **Quien mantenga este documento:** si el Apéndice G crece o se reorganiza, la disciplina de "edición en el sitio" tiene que cubrir explícitamente los prompts de arranque, que son un documento de facto independiente de las secciones de especificación y se desincronizan igual de fácil. |
