...
class Rag::DocumentIdentityScopeTest < ActiveSupport::TestCase
  ...
  test "flag off does not retrieve and a catalog without a printed mark cannot activate" do
    with_flag(nil) do
    ...
    assert_equal 94, catalog.unconfirmed_count
    assert catalog.activatable?

    with_flag("true") do
      assert Rag::DocumentIdentityScope.applicable?(episode)
      Rag::DocumentIdentityCatalog.with_catalog(catalog_for(entry("draft", confirmed: false))) do
        assert_not Rag::DocumentIdentityScope.applicable?(episode)
      end
  ...
  test "lookup key is document_id because the sidecar uri is not the s3 key" do
    ...
    assert_equal 1, entry.evidence_page
    assert_includes entry.evidence_text, "CEA15+"
    assert_equal "component", entry.role
  end
